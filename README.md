...
1. Before starting, verify that your FocalTech fingerprint reader is detected by your system. Open your terminal and run:
```bash
lsusb
```
...
3. Install the standard fingerprint daemon
```bash
...
5. Fix the library path
```bash
sudo cp -a /usr/lib/x86_64-linux-gnu/libfprint-2* /usr/lib/
```

6. Restart the fingerprint daemon
```bash
